...
from nn_model.vae_model_tools import vae_compute_recons_data

from nn_model.clf_tools import get_clf_map_dict, create_data_set_dict, get_stacked_data


class VaeEvalManager(object):
    ...
    def compute_eval_result(self, crf_tr_dataloader, tset_normal_dataloader, test_anomaly_dataloader):
        mid_features = []
        tr_normal_outputs = vae_compute_recons_data(self._ae_model, crf_tr_dataloader, mid_features=mid_features)
        mid_features = []
        normal_outputs = vae_compute_recons_data(self._ae_model, tset_normal_dataloader, mid_features=mid_features)
        mid_features = []
        anomaly_outputs = vae_compute_recons_data(self._ae_model, test_anomaly_dataloader, mid_features=mid_features)
...
class VaeReconstructTest(object):
    def __init__(
            self,
            ae_model,
            output_dir,
...
class TransitCheckManager(object):
    def __init__(
            self,
            ae_model,
            normal_key,
    ...
    def compute_result(self, crf_tr_dataloader, target_dataloader):
        ...
        tr_normal_outputs = vae_compute_recons_data(self._ae_model, crf_tr_dataloader, mid_features=mid_features)
        mid_features = []
        target_outputs = vae_compute_recons_data(self._ae_model, target_dataloader, mid_features=mid_features)
